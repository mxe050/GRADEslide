...
## Step 23: イントロ刷新 + AI/PubMed 比較 + Evidence cycle 図像化 (2026-04-30)

- **イントロ並びを修正**: N3 (イントロダクション) を先頭に、N1 (A薬発表) を 2 枚目に。
  N2 (6 つの確認) は削除。
- **N1**: 演題箇条書きで RCT/SR/MA/CPG の正式名併記。narration の PICO 文を
  「PICO と、そのエビデンスが必要であり、エビデンスの繰り返しではありません」に修正。
- **N3**: タイトル「G先生とは」→「**イントロダクション**」。bullets を 5 行 → 2 行に整理し、
  「Gordon Guyatt 教授の資料を読み込ませた AI で作成した仮想の講師」に書き換え。
- **N4 / N8 / N12 / N16**: 「① 悪い発表例」を「① **よくある発表例**」に統一、N7 の
  comparison leftHeader も同様に。
- **N5**: 末尾に赤字 (em) で「**基礎研究が重要かつ必要なのは、言うまでもない！**」を追加。
- **N6**: narration 末尾に「救急蘇生 CPG ではマネキン・シミュレーション研究が
  エビデンスに使われている」 補足を追加。
- **N9**: NNT 説明後に「最近では NNT より『1000 人中何人がより改善した』 が流行」 補足。
- **N11**: narration 末尾に「癌の化学療法では、第 3 相 RCT による行政の認可で
  『標準治療』 と言われることがある」 補足。
- **N12 / N13 / N15**: garbage in/out を撤去、I² 単独判断の注意を追記、信頼区間を
  異質性指標として明示、5 要因評価の前置きを修正。
- **S1**: narration の「**作る側として**推奨を決定する力 / **Core GRADE** の枠組み」 を
  「**推奨を決定する力** / **EBM の枠組み**」に修正。
- **S4 (slide 25)**: card → **comparison** にレイアウト整形 ((1) AI vs (2) PubMed)。
  narration 末尾に「ちょっとしたトピックスは AI、患者適応は確実な情報を」 補足。
- **S7 (slide 28)**: card 5Steps テキスト → `slideImage` (`/images/slides-full/slide007.jpg`)
  に置換し、循環図がそのまま表示されるよう改善。
- 結果: 総スライド 126 → **125**。version 0.18.0-step23-intro-edits。
